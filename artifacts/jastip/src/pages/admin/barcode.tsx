...
function BarcodeItem({ pkg }: { pkg: any }) {
  ...
  function printBarcode() {
    ...
    if (!win) return;
    win.document
      .write(`<!DOCTYPE html><html><head><title>Barcode - ${pkg.resiNumber}</title>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Cetak atau unduh barcode untuk setiap paket.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm mt-1">
            Tambah paket terlebih dahulu untuk membuat barcode
          </p>
          <Button
            className="mt-4"
            onClick={() => setLocation("/admin/packages/type")}
          >
            Input Paket Baru
          </Button>
        </div>
