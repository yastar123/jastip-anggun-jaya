...
import {
  ArrowLeft, History, Ship, ChevronRight,
  CheckCircle2, Lock, Archive,
} from "lucide-react";
...
const SVC_DEFS = [
  { key: "jastip pesawat", label: "Jastip Pesawat", border: "border-blue-200", bg: "bg-blue-50/60", num: "text-blue-700", badge: "bg-blue-100 text-blue-700 border-blue-200" },
  { key: "jastip hemat+",  label: "Jastip Hemat+",  border: "border-emerald-200", bg: "bg-emerald-50/60", num: "text-emerald-700", badge: "bg-emerald-100 text-emerald-700 border-emerald-200" },
  { key: "jastip kargo",   label: "Jastip Kargo",   border: "border-orange-200", bg: "bg-orange-50/60", num: "text-orange-700", badge: "bg-orange-100 text-orange-700 border-orange-200" },
  { key: "jastip pelni",   label: "Jastip Pelni",   border: "border-indigo-200", bg: "bg-indigo-50/60", num: "text-indigo-700", badge: "bg-indigo-100 text-indigo-700 border-indigo-200" },
];

// ── Main Page ─────────────────────────────────────────────────────────────────

export default function RiwayatPembayaranBatch({ params }: { params: { id: string } }) {
  ...
  const svcGroups = useMemo(() => {
    ...
    for (const pmt of batchPayments) {
      const svc: string =
        ...
        (pmt.packageIds?.[0] != null ? pkgMap[pmt.packageIds[0]]?.serviceType : "") || "";
    ...
    for (const [key, pmts] of map.entries()) {
      if (!knownKeys.includes(key) && pmts.length) {
        result.push({
          def: { key, label: key || "Lainnya", border: "border-gray-200", bg: "bg-gray-50/60", num: "text-gray-700", badge: "bg-gray-100 text-gray-700 border-gray-200" },
          payments: pmts,
        });
      }
    }
  ...
  const totalPiutang = batchPayments.filter(p => p.paymentType === "piutang").reduce((s, p) => s + Number(p.totalAmount || 0), 0);

  function goDetail(serviceTypeKey: string) {
    setLocation(`${base}/riwayat-pembayaran/batch/${batchId}/detail?serviceType=${encodeURIComponent(serviceTypeKey)}`);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        <Button variant="outline" size="icon" onClick={() => setLocation(`${base}/riwayat-pembayaran`)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            <span className="font-semibold text-foreground">{batchPayments.length} transaksi</span>
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {svcGroups.map(({ def, payments: pmts }) => {
            const total = pmts.reduce((s: number, p: any) => s + Number(p.totalAmount || 0), 0);
            const tunai = pmts.filter((p: any) => p.paymentType === "tunai").reduce((s: number, p: any) => s + Number(p.totalAmount || 0), 0);
            const transfer = pmts.filter((p: any) => p.paymentType === "transfer").reduce((s: number, p: any) => s + Number(p.totalAmount || 0), 0);
            const piutang = pmts.filter((p: any) => p.paymentType === "piutang").reduce((s: number, p: any) => s + Number(p.totalAmount || 0), 0);
              ...
              >
                <CardContent className="pt-5 pb-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className={`text-sm font-bold ${def.num}`}>{def.label}</p>
                  ...
                  <div>
                    ...
                    <p className={`text-2xl font-black ${def.num}`}>{formatRp(total)}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{pmts.length} transaksi</p>
                  </div>
                  ...
                  <div className="space-y-1 pt-1 border-t border-border/40">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-semibold">{formatRp(transfer)}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-orange-700 font-medium">Piutang</span>
                      <span className="font-semibold">{formatRp(piutang)}</span>
                    </div>
                    {piutangCount > 0 && (
                      <p className="text-[10px] text-orange-600 pt-0.5">{piutangCount} belum lunas</p>
                    )}
                  </div>
                </CardContent>
              </Card>
